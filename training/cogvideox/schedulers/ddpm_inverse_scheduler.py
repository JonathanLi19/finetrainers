...
class CogvideoXDPMInverseScheduler(SchedulerMixin, ConfigMixin):
    """
    ...
    def _get_variance(self, alpha_prod_t, alpha_prod_t_prev):
        ...
        variance = (beta_prod_t / beta_prod_t_prev) * (1 - alpha_prod_t_prev / alpha_prod_t)
    ...
    def step(
        ...
        if timestep_back is not None:
            timestep_back = min(
                timestep_back - self.config.num_train_timesteps // self.num_inference_steps, self.config.num_train_timesteps - 1
            ) # t-1
        ...
        if timestep_back is not None:
            alpha_prod_t_back = self.alphas_cumprod[timestep_back] if timestep_back >= 0 else self.initial_alpha_cumprod
        else:
        ...
        if old_pred_original_sample is None or prev_timestep >= self.config.num_train_timesteps - 1:
