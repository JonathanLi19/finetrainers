...
@dataclass
class CogVideoXControlnetTransformer3DModelOutput(BaseOutput):
    """
    The output of [`CogVideoXControlnetTransformer3DModel`].
...
class CogVideoXControlnetTransformer3DModel(CogVideoXTransformer3DModel):
    ...
    def __init__(
        ...
        if use_perception_head:
            ...
            self.perception_head = SemanticFPNHead(in_channels=inner_dim, out_channels=self.config.in_channels//2, num_tensors=self.end_layer-self.start_layer+1, patch_size=self.config.patch_size)
        else:
    ...
    def forward(
        ...
        return CogVideoXControlnetTransformer3DModelOutput(sample=output, mask_pred=mask_pred)
    ...
    def from_pretrained(cls, pretrained_model_name_or_path, **kwargs):
        ...
        if unused_kwargs.get("use_perception_head", False):
            ...
            model.perception_head = SemanticFPNHead(in_channels=inner_dim, out_channels=model.config.in_channels//2, num_tensors=model.config.num_layers, patch_size=model.config.patch_size)
