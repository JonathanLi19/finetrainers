...
output_csv = 'evaluation/FID/results/SGI2V_DAVIS.csv'
...
def calculate_fid(video_id):
    command = [
        ...
        "-m",
        "pytorch_fid",
        f"/datadrive2/lqh/generated_videos_as_images/SGI2V_DAVIS/{video_id}",
...
with open(input_csv, 'r') as infile:
    ...
    for row in reader:
        ...
        output_path = f"/datadrive2/lqh/SG-I2V/output_DAVIS/output_videos/{video_id}.mp4"

        # 检查视频文件是否存在以及是否已经处理过
        if os.path.exists(output_path) and video_id not in existing_fid_results:
            save_dir = f"/datadrive2/lqh/generated_videos_as_images/SGI2V_DAVIS/{video_id}"
