...
output_csv = 'evaluation/FID/results/DragAnything_DAVIS.csv'
...
def calculate_fid(video_id):
    command = [
        ...
        "-m",
        "pytorch_fid",
        f"/datadrive3/generated_videos_as_images/DragAnything_DAVIS/{video_id}",
        f"/datadrive3/DAVIS/video_as_images_25frames/{video_id}"
    ]
...
with open(input_csv, 'r') as infile:
    ...
    for row in reader:
        ...
        output_path = f"/home/qid/quanhao/workspace/DragAnything/samples_DAVIS/{video_id}.mp4"

        # 检查视频文件是否存在以及是否已经处理过
        if os.path.exists(output_path) and video_id not in existing_fid_results:
            save_dir = f"/datadrive3/generated_videos_as_images/DragAnything_DAVIS/{video_id}"
