...
output_csv = 'evaluation/FID/results/ImageConductor_DAVIS.csv'
...
def calculate_fid(video_id):
    command = [
        ...
        "-m",
        "pytorch_fid",
        f"/datadrive2/lqh/generated_videos_as_images/ImageConductor_DAVIS/{video_id}",
        f"/datadrive2/lqh/DAVIS/video_as_images_16frames/{video_id}"
    ]
...
with open(input_csv, 'r') as infile:
    ...
    for row in reader:
        ...
        output_path = f"/datadrive2/lqh/ImageConductor/outputs_DAVIS/output_mp4/{video_id}.mp4"

        # 检查视频文件是否存在以及是否已经处理过
        if os.path.exists(output_path) and video_id not in existing_fid_results:
            save_dir = f"/datadrive2/lqh/generated_videos_as_images/ImageConductor_DAVIS/{video_id}"
